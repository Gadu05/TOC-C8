...
public class ProdutoDAO {
    ...
    public int gravar(Produto produto) throws Exception {
        Banco banco;
        int qtde=0;
        try {
            banco = new Banco();
            banco.comando = Banco.conexao.prepareStatement("Insert into produto(descricao, preco, qtde) values (?,?,?)");
            banco.comando.setString(1, produto.getDescricao());
            banco.comando.setDouble(2, produto.getPreco());
            banco.comando.setInt(3, produto.getQtde());
    ...
    public void gravarR(Produto produto) throws Exception {
        Banco banco;
        try {
            banco = new Banco();
            banco.comando = Banco.conexao.prepareStatement("Insert into produto(descricao, preco, qtde) values (?,?,?)", Statement.RETURN_GENERATED_KEYS);
            banco.comando.setString(1, produto.getDescricao());
            banco.comando.setDouble(2, produto.getPreco());
            banco.comando.setInt(3, produto.getQtde());
            ...
            if (banco.tabela.next()) {
                produto.setCodigo(banco.tabela.getInt(1));
                banco.tabela.close();
            } else {
                throw new Exception(" Erro ao capturar Cliente cadastrado.");
            }
            Banco.conexao.close();

        } catch (Exception ex) {
            throw new Exception(" Erro ao gravarR() Cliente: " + ex.getMessage());
        }
    }

    public void gravarR2(Produto produto) {
        Banco banco;

        try {
            banco = new Banco();
            banco.comando = Banco.conexao.prepareStatement("Insert into produto(codigo, decricao, preco, qtde) "
                    + "VALUES (DEFAULT,?,?,?) RETURNING codigo");
            banco.comando.setString(1, produto.getDescricao());
            banco.comando.setDouble(2, produto.getPreco());
            banco.comando.setInt(3, produto.getQtde());
            ...
            Banco.conexao.close();
            banco.tabela.next();
            //Pega o código gerado pelo banco e o coloca no produto
            produto.setCodigo(banco.tabela.getInt("codigo"));
        } catch (Exception ex) {
    ...
    public int alterar(Produto produto) throws Exception {
        ...
        try {
            banco = new Banco();
            banco.comando = Banco.conexao.prepareStatement("Update produto set descricao=?, preco=?, qtde=? where codigo =?");
            banco.comando.setString(1, produto.getDescricao());
            banco.comando.setDouble(2, produto.getPreco());
            banco.comando.setInt(3, produto.getQtde());
            banco.comando.setInt(4, produto.getCodigo());
            qtde= banco.comando.executeUpdate();
            Banco.conexao.close();
            return(qtde);
        } catch (Exception ex) {
    ...
    public int remover(int  codigo) throws Exception {
        ...
        try {
            banco = new Banco();
            banco.comando = Banco.conexao.prepareStatement("Delete from produto where codigo =?");
            banco.comando.setInt(1, codigo);
            qtde= banco.comando.executeUpdate();
            Banco.conexao.close();
            return(qtde);
        } catch (Exception ex) {
            throw new Exception("Erro ao remover Cliente: " + ex.getMessage());
        }
    }
    

    public Produto preencher(int codigo) throws Exception {
        Banco banco;
            Produto produto = null;
        try {
            banco = new Banco();
            banco.comando = Banco.conexao.prepareStatement(
                    "Select codigo,descricao,preco,qtde from produto where codigo=?");
            banco.comando.setInt(1, codigo);
            banco.tabela = banco.comando.executeQuery();
            if ((banco.tabela != null) && (banco.tabela.next())) {
                produto = new Produto();
                produto.setCodigo(banco.tabela.getInt(1));
                produto.setDescricao(banco.tabela.getString(2));
                produto.setPreco(banco.tabela.getDouble(3));
                produto.setQtde(banco.tabela.getInt(4));
            } 
            Banco.conexao.close();
            return(produto);
        } catch (Exception ex) {
    ...
    public ResultSet listar() throws Exception {
        ...
        try {
            ...
            banco.comando = Banco.conexao.prepareStatement("Select codigo,descricao,preco,qtde from produto", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            banco.tabela = banco.comando.executeQuery();
            return (banco.tabela);
        } catch (Exception ex) {
    ...
    public String geraSql(int codigo, String nome, int idade) {
        ...
        try {
            ...
            if (aux.length() > 0) {
                aux = "Select codigo, descricao, preco, qtde from produto " + aux;
            } else {
                aux = "Select codigo, descricao, preco, qtde from produto ";
            }
            return (aux);
        } catch (Exception ex) {
    ...
    */
    ...
    public ResultSet consultarVariosCamposSimples(int codigo, String nome, int idade) throws Exception {
        ...
        try {
            ...
            if (aux.length() > 0) {
                aux = "Select codigo, nome, idade from produto " + aux;
            } else {
                aux = "Select codigo, nome, idade from produto ";
            }
